...
const FILTER_CATEGORIES = {
  "콘센트": ["넉넉함", "거의 전좌석", "살짝 부족", "거의 없음"],
  "분위기": ["카공러 다수", "카공러 적지만"],
  "소음":   ["조용함", "대화 반 작업 반", "대화 위주"],
  "공간":   ["감성", "예쁨", "특이", "아늑", "뷰 맛집", "충고 높음", "통창"],
  "테이블": ["네모"],
  "메뉴":   ["맛있는 음료", "맛있는 디저트", "식사 대용 끼니"],
};
...
function TagChip({ label, selected, onClick, size = "md" }) {
  ...
  const colors = selected
    ? { background: PURPLE_LIGHT, border: "1.5px solid #C4B5FD", color: PURPLE_DARK }
    : { background: "#F5F4FF", border: "1.5px solid #D8D4F0", color: "#888" };
  return (
    <button onClick={onClick} style={{ ...sizes[size], ...colors, display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 20, outline: "none", WebkitTapHighlightColor: "transparent", cursor: onClick ? "pointer" : "default", fontFamily: "inherit", fontWeight: 600, transition: "background 0.15s, color 0.15s, border 0.15s", whiteSpace: "nowrap", lineHeight: 1.2 }}>{label}</button>
  );
}
